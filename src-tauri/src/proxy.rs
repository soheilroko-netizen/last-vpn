...
self.debug_log(format!("config check FAILED: {}{}", err_text.trim(), out_text.trim()));
bail!(
...
stls_ips.iter().map(|ip| format!("{}/32", ip)).collect()
};
// FIX v2: also bypass the remote DNS server (8.8.8.8) via direct route
...
server_port: Some(53),
// FIX v2: use direct detour for DNS (not through proxy) and use UDP
// TCP 53 is blocked by many ISPs/firewalls; UDP 53 is standard.
...
// FIX v2: use /24 instead of /30 — /30 only allows 2 usable hosts
inbounds: vec![SbInbound {
typ: "tun".into(),
tag: "tun-in".into(),
listen: None,
...
SbRouteRule {
protocol: None,
// Bypass both server IPs and the remote DNS server directly
...
rules: Some(vec![SbDnsRule { server: Some("dns-remote".into()) }]),
strategy: Some("prefer_ipv4".into()),
}),
inbounds: vec![],
outbounds: vec![],
